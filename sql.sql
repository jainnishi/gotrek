...
insert  into `trek_info`(`trek_id`,`trek_name`,`start_date`,`end_date`,`meetup_point`,`category`,`Notes`,`img_url`,`treksummary`) values (1,'Rajmanchi','2014-11-29','2014-11-30','Karjat','music',NULL,'formats/images/t17.jpg','PMT: Trek to Rajmachi on 29th - 30th Nov 14.'),(2,'Peb Fort','2014-11-26','2014-11-30','Peb Fort','spiritual',NULL,'formats/images/t18.jpg','PMT: Trek to Peb Fort on 26th Oct 14.'),(3,'Netrani island','2015-01-24','2015-01-26','Island meetup','adventure',NULL,'formats/images/t19.jpg','PMT: Scuba Diving Special Visit to Netrani Island,on 24th January to 26th January 2015.'),(4,'Dudhsagar waterfall','2015-01-17','2015-01-18','Island meetup','nature lover',NULL,'formats/images/t20.jpg','PMT:Trek to Dudhsagar Waterfall Top on 17th and 18th January 2015');

/*Table structure for table `trekuser_xref` */

DROP TABLE IF EXISTS `trekuser_xref`;

CREATE TABLE `trekuser_xref` (
  `xref_id` int(11) NOT NULL AUTO_INCREMENT,
  `prefix` char(30) NOT NULL DEFAULT 'R',
  `trek_id` int(11) NOT NULL,
  `user_id` int(11) NOT NULL,
  `role` varchar(20) NOT NULL DEFAULT 'member',
  PRIMARY KEY (`xref_id`,`prefix`),
  UNIQUE KEY `unkqkey` (`trek_id`,`user_id`),
  KEY `user_id` (`user_id`),
  CONSTRAINT `trekuser_xref_ibfk_1` FOREIGN KEY (`user_id`) REFERENCES `user_info` (`id`) ON DELETE CASCADE ON UPDATE CASCADE,
  CONSTRAINT `trekuser_xref_ibfk_2` FOREIGN KEY (`trek_id`) REFERENCES `trek_info` (`trek_id`) ON DELETE CASCADE ON UPDATE CASCADE
) ENGINE=InnoDB AUTO_INCREMENT=11 DEFAULT CHARSET=latin1;

/*Data for the table `trekuser_xref` */

insert  into `trekuser_xref`(`xref_id`,`prefix`,`trek_id`,`user_id`,`role`) values (4,'R',1,6,'member'),(8,'R',2,6,'member'),(10,'R',3,6,'member');
